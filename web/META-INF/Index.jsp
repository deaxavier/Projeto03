<%-- 
    Document   : Index
    Created on : 16/10/2018, 12:34:46
    Author     : Victoria.Rodovalho
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head lang="pt-br">
		<title>Projeto 3 - P.O.O</title>
		<meta charset="utf-8">
		<link rel="stylesheet" type="text/css" href="css/reset.css">
		<link rel="stylesheet" type="text/css" href="css/sobremim.css">
		<link href="https://fonts.googleapis.com/css?family=Crimson+Text|Montserrat|Raleway" rel="stylesheet">
	</head>
	<body>
	   <main>
		<h1>Projeto 3 - Cadastro</h1>
		<div>
                    <p>Projeto de Programacao Orientada a Objetos</p>
                    <h2>Integrantes</h2>
			<p>
			   <ul>
                                <li>Andrea Xavier</li>
                 		<li>Arthur de la Mano</li>
                 		<li>Lucas Martins</li>
                 		<li>Victoria Rodovalho</li>
                            </ul>
         		</p>

                    <h3>Finalidade</h3>
                    <p>
                        <ul>
                            <li>
				Pagina de manutencao para <strong>cadastro de pessoas</strong>  contendo nome, CPF, e-mail e telefone.
                            </li>

                            <li>
				Pagina de manutencao para <strong>cadastro de empresas</strong> contendo nome, CNPJ, razao social, telefone e website.
                            </li>

                            <li>
				Pagina de manutencao <strong>para cadastro de veiculos</strong> contento placa, marca, modelo e cor.
                            </li>
                        </ul>
                    </p>
		</div>
	   </main>
	</body>
	<aside>
            <nav class="navegacao-site">
                <ul>
                    <h1>Menu</h1>
                    <li><a href="index.html">Cadastro Pessoa</a></li>
                    <li><a href="sobremim.html">Cadastro Empresa</a></li>
                    <li><a href="portfolio.html">Cadastro Veiculo</a></li>
                </ul>
            </nav>
            <nav class="icones-sociais">
        	<ul>
                    <li><a class="github" href="https://github.com/deaxavier/Projeto03" target="_blank">GitHub</a></li>
        	</ul>
            </nav>
	</aside>

	<footer>
		&copy Andrea, Arthur, Lucas e Victoria - 2018
	</footer>
</html>
